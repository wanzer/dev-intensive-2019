...
class GroupViewModel: ViewModel() {
    ...
    private val selectItems = Transformations.map(userItems){users -> users.filter {it.isSelected}}
    ...
    fun getUserData(): LiveData<List<UserItem>> {
        ...
        val filterF = {
            ...
            else users.filter { it.fullName.contains(queryStr, false) }
        }
    ...
    fun getSelectedData(): LiveData<List<UserItem>> = selectItems
    ...
    fun handleCreateGroup() {
        groupRepository.createChat(selectItems.value!!)
    }
}